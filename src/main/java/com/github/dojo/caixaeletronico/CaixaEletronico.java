...
public class CaixaEletronico {
    ...
    public Integer sacar(BigDecimal valorSacar) {


        if (valorSacar.equals(new BigDecimal(0)) || valorSacar.intValue() < 0) {
            throw new IllegalArgumentException();
        }

        if (valorSacar.remainder(new BigDecimal(2)).intValue() != 0){
            throw new IllegalArgumentException();
        }



        BigDecimal qtdNotas =  valorSacar.divide(new BigDecimal(100));
